  ...
  $(function () {
    ...
    QUnit.test('should hide modal when hide is called', function (assert) {
      ...
      $('<div id="modal-test"/>').appendTo('#qunit-fixture').on('shown.bs.modal', function () {
        ...
        assert.notEqual($('#modal-test').length, 0, 'modal inserted into dom');
        $(this || _global).bootstrapModal('hide');
    ...
    QUnit.test('should toggle when toggle is called', function (assert) {
      ...
      $('<div id="modal-test"/>').appendTo('#qunit-fixture').on('shown.bs.modal', function () {
        assert.ok($('#modal-test').is(':visible'), 'modal visible');
        assert.notEqual($('#modal-test').length, 0, 'modal inserted into dom');
        $(this || _global).bootstrapModal('toggle');
      }).on('hidden.bs.modal', function () {
        assert.ok(!$('#modal-test').is(':visible'), 'modal hidden');
        done();
    ...
    QUnit.test('should remove from dom when click [data-dismiss="modal"]', function (assert) {
      assert.expect(3);
      var done = assert.async();
      $('<div id="modal-test"><span class="close" data-dismiss="modal"/></div>').appendTo('#qunit-fixture').on('shown.bs.modal', function () {
        assert.ok($('#modal-test').is(':visible'), 'modal visible');
        assert.notEqual($('#modal-test').length, 0, 'modal inserted into dom');
        $(this || _global).find('.close').trigger('click');
      }).on('hidden.bs.modal', function () {
        assert.ok(!$('#modal-test').is(':visible'), 'modal hidden');
        done();
      }).bootstrapModal('toggle');
    ...
    QUnit.test('should close modal when clicking outside of modal-content', function (assert) {
      assert.expect(3);
      var done = assert.async();
      $('<div id="modal-test"><div class="contents"/></div>').appendTo('#qunit-fixture').on('shown.bs.modal', function () {
        assert.notEqual($('#modal-test').length, 0, 'modal inserted into dom');
    ...
    QUnit.test('should close reopened modal with [data-dismiss="modal"] click', function (assert) {
      assert.expect(2);
      var done = assert.async();
      $('<div id="modal-test"><div class="contents"><div id="close" data-dismiss="modal"/></div></div>').appendTo('#qunit-fixture').one('shown.bs.modal', function () {
        $('#close').trigger('click');
      }).one('hidden.bs.modal', function () {
        // after one open-close cycle
        assert.ok(!$('#modal-test').is(':visible'), 'modal hidden');
        $(this || _global).one('shown.bs.modal', function () {
          $('#close').trigger('click');
        }).one('hidden.bs.modal', function () {
          assert.ok(!$('#modal-test').is(':visible'), 'modal hidden');
          done();
    ...
    QUnit.test('should restore focus to toggling element when modal is hidden after having been opened via data-api', function (assert) {
      ...
      $('<div id="modal-test"><div class="contents"><div id="close" data-dismiss="modal"/></div></div>').appendTo('#qunit-fixture').on('hidden.bs.modal', function () {
        setTimeout(function () {
    ...
    QUnit.test('should not restore focus to toggling element if the associated show event gets prevented', function (assert) {
      ...
      $('<div id="modal-test"><div class="contents"><div id="close" data-dismiss="modal"/></div>').appendTo('#qunit-fixture').one('show.bs.modal', function (e) {
        e.preventDefault();
        $otherBtn.trigger('focus');
        setTimeout($.proxy(function () {
          $(this || _global).bootstrapModal('show');
        }, this || _global), 0);
      }).on('hidden.bs.modal', function () {
        setTimeout(function () {
    ...
    QUnit.test('should add padding-right of scrollbar width to .navbar-fixed-top and .navbar-fixed-bottom before open', function (assert) {
      ...
      var done = assert.async();
      var $body = $(document.body);
      var scrollbarWidth; // simulate overflow scroll

      $body.css({
        height: '2000px'
      });
      var $fixedTop = $('<nav class="navbar navbar-fixed-top" />').appendTo($body);
      var $fixedBottom = $('<nav class="navbar navbar-fixed-bottom" />').appendTo($body); // patch setScrollbar function to get scrollbar width

      var setScrollbar = Modal.prototype.setScrollbar;

      Modal.prototype.setScrollbar = function () {
        setScrollbar.call(this || _global);
        scrollbarWidth = (this || _global).scrollbarWidth + 'px';
      };

      $('<div id="modal-test"/>').on('hidden.bs.modal', function () {
        $fixedTop.remove();
        $fixedBottom.remove();
        $body.removeAttr('style'); // restore original setScrollbar
        ...
        assert.strictEqual(scrollbarWidth, fixedTopPaddingRight, '.navbar-fixed-top has padding-right (' + fixedTopPaddingRight + ') equal to scrollbar width (' + scrollbarWidth + ')');
        assert.strictEqual(scrollbarWidth, fixedBottomPaddingRight, '.navbar-fixed-bottom has padding-right (' + fixedBottomPaddingRight + ') equal to scrollbar width (' + scrollbarWidth + ')');
        $(this || _global).bootstrapModal('hide');
      }).bootstrapModal('show');
    });
    QUnit.test('should restore inline padding-right for .navbar-fixed-top and .navbar-fixed-bottom after close', function (assert) {
      ...
      var $fixedBottom = $('<nav class="navbar navbar-fixed-bottom" style="padding-right: 5%;" />').appendTo($body); // simulate overflow scroll

      $body.css({
        height: '2000px'
      });
      $('<div id="modal-test"/>').on('hidden.bs.modal', function () {
        assert.strictEqual($fixedTop.css('padding-right'), '10px', '.navbar-fixed-top has inline padding-right restored');
        assert.strictEqual($fixedBottom[0].style.paddingRight, '5%', '.navbar-fixed-bottom has right padding-right restored');
        $fixedTop.remove();
        $fixedBottom.remove();
        $body.removeAttr('style');
        $styleshhet.remove();
